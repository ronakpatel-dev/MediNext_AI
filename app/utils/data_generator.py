...
def generate_appointment_data(num_appointments=30):
    ...
    appointment_types = ["Consultation", "Follow-up", "New Patient", "Emergency", "Routine Check"]
...
def generate_prescription_data(num_prescriptions=40):
    ...
    medications = [
        ...
        "Losartan", "Simvastatin", "Metoprolol", "Hydrochlorothiazide", "Sertraline",
        "Albuterol", "Ibuprofen", "Acetaminophen", "Aspirin", "Warfarin"
    ]
    
    dosages = [
        "10mg daily", "20mg daily", "40mg daily", "500mg twice daily", "1000mg daily",
        "25mg daily", "50mg daily", "75mg daily", "100mg daily", "5mg daily"
    ]
    
    frequencies = [
        "Once daily", "Twice daily", "Three times daily", "As needed", "Every 8 hours",
        "Every 12 hours", "Weekly", "Monthly"
    ]
...
def generate_activity_data(num_activities=25):
    ...
    activity_types = [
        "lab_result", "appointment", "prescription", "alert", "questionnaire",
        "check_in", "check_out", "test_result", "referral", "note"
    ]
    ...
    for i in range(num_activities):
        ...
        time_ago = f"{hours_ago}h {minutes_ago}m ago" if hours_ago > 0 else f"{minutes_ago}m ago"
        
        activity_type = random.choice(activity_types)
        
        # Generate activity description based on type
        if activity_type == "lab_result":
            activity = f"New lab result uploaded for Patient P{random.randint(1, 50):03d}"
        elif activity_type == "appointment":
            activity = f"Appointment scheduled for tomorrow at {random.randint(8, 17)}:00"
...
def generate_analytics_data():
    ...
    base_visits = 15
    patient_visits = np.random.poisson(base_visits, len(dates)) + np.random.normal(0, 2, len(dates))
    ...
    dept_df = pd.DataFrame({
        ...
        'Patient Count': department_counts
    })
    
    # Weekly pattern
    weekly_visits = visits_df.groupby(visits_df['Date'].dt.dayofweek)['Patient Visits'].mean()
    days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    
    weekly_df = pd.DataFrame({
        'Day': days,
        'Average Visits': weekly_visits.values
    })
