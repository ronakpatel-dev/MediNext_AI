...
def create_activity_feed():
    ...
    for i, activity in enumerate(activities):
        ...
        with col1:
            st.markdown(f"<div class='activity-icon' style='background-color: #f3f4f6; color: #6b7280;'>{icon}</div>", unsafe_allow_html=True)
        
        with col2:
            st.markdown(f"<p class='activity-text'>{activity['activity']}</p>", unsafe_allow_html=True)
            st.markdown(f"<p class='activity-time'>{activity['time']}</p>", unsafe_allow_html=True)
        
        with col3:
            st.markdown(f"<span class='activity-priority {priority_class}'>{activity['priority']}</span>", unsafe_allow_html=True)
