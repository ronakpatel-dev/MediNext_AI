...
def load_patient_data():
    ...
    try:
        ...
        if os.path.exists(csv_path):
            ...
            st.info("📁 No CSV file found. Using sample data. Place synthetic_ehr_data.csv in app/assets/ to use real data.")
            return get_sample_data()
...
def validate_patient_csv(df):
    ...
    required_columns = [
        'patient_id', 'name', 'sex', 'birth_date', 'address', 
        'last_visit', 'conditions', 'medications', 'hemoglobin', 
        'glucose', 'ssn', 'phone_number', 'guardrail_violation_flag'
    ]
    ...
    if empty_critical.any():
        st.warning(f"⚠️ {empty_critical.sum()} patients have missing critical information")
...
def get_sample_data():
    ...
    return pd.DataFrame({
        ...
        'last_name': ['Doe', 'Smith', 'Johnson', 'Wilson', 'Brown'], # Will be ignored
        'name': ['John A Doe', 'Jane B Smith', 'Mike C Johnson', 'Sarah D Wilson', 'David E Brown'],
        'sex': ['M', 'F', 'M', 'F', 'M'],  # Will be displayed as 'gender'
        'birth_date': ['1979-05-15', '1992-08-20', '1966-03-10', '1995-11-25', '1957-12-03'],
        'address': ['123 Main St', '456 Oak Ave', '789 Pine Rd', '321 Elm St', '654 Maple Dr'],
        'last_visit': ['2024-01-15', '2024-01-20', '2024-01-18', '2024-01-22', '2024-01-19'],
        'conditions': ['Hypertension', 'Diabetes', 'Asthma', 'None', 'Arthritis'],  # Will be displayed as 'medical_conditions'
        'medications': ['Lisinopril', 'Metformin', 'Albuterol', 'None', 'Ibuprofen'],
...
def create_patient_table():
    ...
    with col2:
        ...
        if 'last_visit' in df.columns:
            try:
                ...
                active_patients = len(last_visit_dates[last_visit_dates >= datetime.now() - timedelta(days=365)])
    ...
    display_columns = ['patient_id', 'name', 'sex', 'birth_date', 'last_visit', 'conditions', 'medications', 'glucose', 'hemoglobin', 'guardrail_violation_flag']
    ...
    display_df['guardrail_violation_flag'] = display_df['guardrail_violation_flag'].map({True: 'Yes', False: 'No'})
    ...
    page_size = st.selectbox("Rows per page", [10, 20, 50, 100], index=[10, 20, 50, 100].index(st.session_state.page_size), key="page_size_selector")
    ...
    with col3:
        if st.button("Next ➡️", disabled=st.session_state.current_page_number >= total_pages):
            st.session_state.current_page_number += 1
            st.rerun()
    ...
    if event.selection.rows:
        ...
        logger.info(f"Selected patient data: {st.session_state.selected_patient_data.to_dict('records')}")
