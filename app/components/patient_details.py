...
def call_superwise_api(patient_data):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Superwise API call successful for patient: {patient_data['patient_id']}")
            return {
                "success": True,
                "data": api_response,
            ...
            logger.error(f"Superwise API error: {response.status_code} - {response.text}")
            return {
                "success": False,
...
def create_patient_details():
    ...
    if 'selected_patient_data' in st.session_state and st.session_state.selected_patient_data is not None:
        ...
        patient_data = {
            ...
            'first_name': selected_patient['name'].split()[0] if len(selected_patient['name'].split()) > 0 else '',
            'middle_initial': selected_patient['name'].split()[1] if len(selected_patient['name'].split()) > 1 else '',
            'last_name': selected_patient['name'].split()[2] if len(selected_patient['name'].split()) > 2 else '',
            ...
            'guardrail_violation_flag': selected_patient.get('guardrail_violation_flag', 'No') == 'Yes'
        }
    else:
    ...
    with col1:
        ...
        st.write(f"{patient_data['first_name']} {patient_data['middle_initial']} {patient_data['last_name']}")
    ...
    with col2:
        ...
        if patient_data['guardrail_violation_flag']:
            st.markdown('<span class="status-badge status-violation">Violation Detected</span>', unsafe_allow_html=True)
        else:
            st.markdown('<span class="status-badge status-clear">Clear</span>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        if pd.isna(medications_value) or medications_value == 'None' or medications_value == '' or str(medications_value).lower() == 'nan':
            st.write("-")
        else:
    ...
    with col2:
        if 'superwise_response' in st.session_state:
            ...
            with st.container():
                ...
                guardrail_status = '⚠️ Violation Detected' if patient_data['guardrail_violation_flag'] else '✅ Clear'
                
                st.markdown(f"**Guardrail Status:** {guardrail_status}", unsafe_allow_html=True)
                
                st.markdown("---")
        else:
            st.info("Click 'Ask Superwise' to get AI-powered insights about this patient.")
    
    # Close the container div
    st.markdown("</div>", unsafe_allow_html=True)
    
    # Log the component rendering
    logger.info(f"Patient details component rendered for patient: {patient_data['patient_id']}")
...
def show_patient_details_page():
    ...
    with col2:
        st.markdown("<br>", unsafe_allow_html=True)  # Add some spacing to align with title
