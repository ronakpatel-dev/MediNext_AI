...
def calculate_patient_metrics(df):
    ...
    # Condition counts
    diabetes_patients = len(df[df['conditions'].str.contains('diabetes', case=False, na=False)])
    hypertension_patients = len(df[df['conditions'].str.contains('hypertension', case=False, na=False)])
...
def create_dashboard_cards():
    ...
    with col4:
        ...
        <div class="metric-card" style="border-left-color: #ef4444;">
            <div class="card-header">
                ...
                <div>
                    <p class="card-title">Critical Alerts</p>
                </div>
            </div>
            <h2 class="card-value">{metrics['critical_alerts']}</h2>
            <p class="card-change">🔴 Requires attention</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        <div class="metric-card" style="border-left-color: #f97316;">
            <div class="card-header">
                <div class="card-icon" style="background-color: #fed7aa; color: #ea580c;">
                    🛡️
                </div>
                <div>
                    <p class="card-title">Guardrail Violations</p>
                </div>
            </div>
            <h2 class="card-value">{metrics['guardrail_violations']}</h2>
            <p class="card-change">⚠️ {metrics['guardrail_violations']/metrics['total_patients']*100:.1f}% of patients</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        <div class="metric-card" style="border-left-color: #84cc16;">
            <div class="card-header">
                ...
                <div>
                    <p class="card-title">Active Patients</p>
                </div>
            </div>
            <h2 class="card-value">{metrics['active_patients']}</h2>
            <p class="card-change">✅ {metrics['active_patients']/metrics['total_patients']*100:.1f}% engagement</p>
        </div>
        """, unsafe_allow_html=True)
