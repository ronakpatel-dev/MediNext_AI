...
def analyze_patient_data(df):
    ...
    visit_by_month = recent_visits.groupby(recent_visits['last_visit'].dt.to_period('M')).size()
    
    # Health metrics distribution
    glucose_ranges = [0, 70, 100, 126, 200, 300]
    glucose_labels = ['Low', 'Normal', 'Pre-diabetes', 'Diabetes', 'High']
    df['glucose_category'] = pd.cut(df['glucose'], bins=glucose_ranges, labels=glucose_labels, right=False)
    ...
    df['hemoglobin_category'] = pd.cut(df['hemoglobin'], bins=hemoglobin_ranges, labels=hemoglobin_labels, right=False)
...
def create_analytics_charts():
    ...
    with col2:
        ...
        summary_data = {
            'Metric': ['Total Patients', 'Average Age', 'Diabetes Patients', 'Hypertension Patients', 'Critical Alerts'],
            'Value': [
                analysis['total_patients'],
                f"{analysis['avg_age']:.1f} years",
                len(df[df['conditions'].str.contains('diabetes', case=False, na=False)]),
                len(df[df['conditions'].str.contains('hypertension', case=False, na=False)]),
                len(df[(df['glucose'] > 200) | (df['hemoglobin'] < 12) | (df['guardrail_violation_flag'] == True)])
            ]
        }
